...
docker build `
	-t screenly-differ:$($buildNumber)-staging $buildPath

# Tag the current image with specific BUILD_NUMBER version

docker tag screenly-differ:$($buildNumber)-staging 779051441487.dkr.ecr.eu-west-1.amazonaws.com/screenly-differ:$($buildNumber)-staging
...
if ($push)
{
	...
	docker push 779051441487.dkr.ecr.eu-west-1.amazonaws.com/screenly-differ:$($buildNumber)-staging
}
